...
def test_k1_long_query_split_fallback(monkeypatch):
    ...
    monkeypatch.setattr(memory_v5, "store", _kw_store_factory(records))
    # 整句 miss → 0 命中 → 触发兜底
...
def test_k2_no_fallback_when_enough(monkeypatch):
    ...
    monkeypatch.setattr(memory_v5, "store", _kw_store_factory([FakeMem(9, "memU 额外")]))
...
def test_k5_store_error_fail_open(monkeypatch):
    ...
    monkeypatch.setattr(memory_v5, "store", Boom)

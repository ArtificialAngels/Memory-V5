...
def test_u2_lexical_hit_and_fallback(monkeypatch):
    ...
    monkeypatch.setattr(memory_v5, "store", fake_empty)
    monkeypatch.setattr(mr, "retrieve", _semantic)
...
def test_u4_tree_scope(monkeypatch):
    ...
    monkeypatch.setattr("memory_v5.extensions.tree_adapter.tree_scoped_retrieve",
                        fake_tree_scoped)
    ft = FakeTree()
...
def test_u6_dedup_and_norm(monkeypatch):
    ...
    monkeypatch.setattr("memory_v5.search.entity_graph_search",
                        lambda q, top_k=5, **kw: [{
...
def test_u7_fail_open(monkeypatch):
    ...
    monkeypatch.setattr("memory_v5.search.entity_graph_search", boom)
    monkeypatch.setattr(memory_v5, "store",
